...
from sklearn.cluster import AgglomerativeClustering, SpectralClustering, MeanShift, Birch
...
def dynamic_clustering(embeddings, scaled_embeddings):
    """
    Automatically determine optimal number of clusters and method with error handling
    """
    # Test different numbers of clusters
    n_clusters_range = range(3, 10)  # Reduced range
    results = {}
    ...
    for n_clusters in n_clusters_range:
        ...
        methods = {
            'spectral': SpectralClustering(
                ...
                random_state=42,
                affinity='nearest_neighbors',  # Changed from default
                n_neighbors=10  # Added parameter
            ),
            'agglomerative': AgglomerativeClustering(
                n_clusters=n_clusters,
                linkage='ward'  # Added parameter
            ),
            'gmm': GaussianMixture(
        ...
        for name, clusterer in methods.items():
            try:
                ...
                # Check if we got more than one cluster
                if len(np.unique(labels)) > 1:
                    score = silhouette_score(scaled_embeddings, labels)
                    results[f"{name}_{n_clusters}"] = (labels, score)
            except Exception as e:
                print(f"Warning: {name} clustering with {n_clusters} clusters failed: {str(e)}")
                continue
    
    if not results:
        # Fallback to simple clustering if all others fail
    ...
    return best_method[1][0]  # Return labels of best method

def consensus_clustering(embeddings, scaled_embeddings):
    """
    Modified consensus clustering with error handling
    """
    # Get labels from different methods with error handling
    labels_list = []
    ...
    # Count how often pairs of points are clustered together
    ...
    # Normalize
    co_occurrence /= max(len(labels_list), 1)
    
    # Final clustering on co-occurrence matrix
    try:
        final_clustering = AgglomerativeClustering(n_clusters=5)
        consensus_labels = final_clustering.fit_predict(co_occurrence)
        return consensus_labels
    except:
        print("Falling back to simple clustering")
        return AgglomerativeClustering(n_clusters=5).fit_predict(scaled_embeddings)

def two_level_clustering(embeddings, scaled_embeddings):
    """
    Modified two-level clustering with error handling
    """
    try:
        # First level: broad viewpoints
        first_level = AgglomerativeClustering(n_clusters=3)
        major_labels = first_level.fit_predict(scaled_embeddings)
        
        # Second level: detailed positions within each viewpoint
        final_labels = np.zeros_like(major_labels)
        
        for i in range(3):
            mask = major_labels == i
            if np.sum(mask) > 3:
                sub_clustering = AgglomerativeClustering(
                    n_clusters=min(3, np.sum(mask)-1)
                )
                sub_labels = sub_clustering.fit_predict(scaled_embeddings[mask])
                final_labels[mask] = sub_labels + (i * 3)
        
        return final_labels
    except Exception as e:
        print(f"Two-level clustering failed: {str(e)}")
        print("Falling back to simple clustering")
        return AgglomerativeClustering(n_clusters=5).fit_predict(scaled_embeddings)
...
# Try all methods
two_level_labels = two_level_clustering(embeddings, scaled_embeddings)
consensus_labels = consensus_clustering(embeddings, scaled_embeddings)
dynamic_labels = dynamic_clustering(embeddings, scaled_embeddings)

# Compare results
methods = {
    'Two-Level': two_level_labels,
    'Consensus': consensus_labels,
    'Dynamic': dynamic_labels
}

# Visualize and analyze each method
for name, labels in methods.items():
    print(f"\n{name} Clustering Results:")
    print(f"Number of clusters: {len(np.unique(labels))}")
    print(f"Silhouette Score: {silhouette_score(scaled_embeddings, labels):.3f}")
    ...
    for cluster_id in sorted(set(labels)):
        print(f"\nCluster {cluster_id} contents:")
        cluster_indices = np.where(labels == cluster_id)[0]
        print(f"Size: {len(cluster_indices)} statements")
        # Print first statement from each cluster
        if len(cluster_indices) > 0:
            print("Example statement:")
            print(politician_responses[cluster_indices[0]])