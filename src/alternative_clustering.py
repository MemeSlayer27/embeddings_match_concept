...
def plot_dendrogram(model, **kwargs):
    """Create linkage matrix and plot dendrogram for hierarchical clustering"""
...
# After your DBSCAN clustering, add these:
...
agg = AgglomerativeClustering(n_clusters=10, compute_distances=True)
agg_labels = agg.fit_predict(scaled_embeddings)
...
for i in range(len(np.unique(agg_labels))):
    print(f"Cluster {i}: {np.sum(agg_labels == i)} points")

# Visualize clusters
visualize_clusters(embeddings, agg_labels, method='tsne')
print(f"Silhouette Score: {silhouette_score(scaled_embeddings, agg_labels):.3f}")
...
spectral_labels = spectral.fit_predict(scaled_embeddings)

print(f"Number of clusters: {len(np.unique(spectral_labels))}")
for i in range(len(np.unique(spectral_labels))):
    print(f"Cluster {i}: {np.sum(spectral_labels == i)} points")

visualize_clusters(embeddings, spectral_labels, method='tsne')
print(f"Silhouette Score: {silhouette_score(scaled_embeddings, spectral_labels):.3f}")
...
gmm_labels = gmm.fit_predict(scaled_embeddings)

print(f"Number of clusters: {len(np.unique(gmm_labels))}")
for i in range(len(np.unique(gmm_labels))):
    print(f"Cluster {i}: {np.sum(gmm_labels == i)} points")
    
# Plot cluster probabilities
plt.figure(figsize=(15, 5))
plt.imshow(gmm.predict_proba(scaled_embeddings).T, aspect='auto', cmap='viridis')
...
visualize_clusters(embeddings, gmm_labels, method='tsne')
print(f"Silhouette Score: {silhouette_score(scaled_embeddings, gmm_labels):.3f}")
...
meanshift = MeanShift(bandwidth=40)
ms_labels = meanshift.fit_predict(scaled_embeddings)

print(f"Number of clusters: {len(np.unique(ms_labels))}")
for i in range(len(np.unique(ms_labels))):
    print(f"Cluster {i}: {np.sum(ms_labels == i)} points")

visualize_clusters(embeddings, ms_labels, method='tsne')
print(f"Silhouette Score: {silhouette_score(scaled_embeddings, ms_labels):.3f}")
...
birch_labels = birch.fit_predict(scaled_embeddings)

print(f"Number of clusters: {len(np.unique(birch_labels))}")
for i in range(len(np.unique(birch_labels))):
    print(f"Cluster {i}: {np.sum(birch_labels == i)} points")

visualize_clusters(embeddings, birch_labels, method='tsne')
print(f"Silhouette Score: {silhouette_score(scaled_embeddings, birch_labels):.3f}")
...
for method_name, method_labels in methods.items():
    n_clusters = len(np.unique(method_labels)) - (1 if -1 in method_labels else 0)
    if -1 in method_labels:
        n_noise = np.sum(method_labels == -1)
    else:
        n_noise = 0
    
    silhouette = silhouette_score(scaled_embeddings, method_labels) if n_clusters > 1 else 0
    ...
    for i in range(n_clusters):
        print(f"Cluster {i}: {np.sum(method_labels == i)} points")